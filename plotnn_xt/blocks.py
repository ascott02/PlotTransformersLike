...
def encoder_block_factory(width_override: float | None = None):
    ...
    def build(idx: int, x: float, y: float) -> Tuple[List[Node], List[Edge], float]:
        ...
        ln1 = layernorm(f"ln{idx}_1", x, y)
        att = mha(f"mha{idx}", x + 3.2, y)
        add1 = residual_add(f"add{idx}_1", x + 5.6, y)
        ln2 = layernorm(f"ln{idx}_2", x + 7.3, y)
        ff = ffn(f"ffn{idx}", x + 10.0, y + 0.2)
        add2 = residual_add(f"add{idx}_2", x + 12.4, y + 0.2)
...
def decoder_block_factory(width_override: float | None = None, include_cross: bool = True):
    ...
    def build(idx: int, x: float, y: float):
        ln1 = layernorm(f"dln{idx}_1", x, y)
        mmha = mha(f"dmha{idx}", x + 3.2, y, masked=True)
        add1 = residual_add(f"dadd{idx}_1", x + 5.6, y)
        nodes = [ln1, mmha, add1]
        edges = [
        ...
        cursor_x = add1.x + 1.7
        if include_cross:
            ln2 = layernorm(f"dln{idx}_2", cursor_x, y)
            xatt = cross_attn(f"xatt{idx}", cursor_x + 3.2, y)
            add2 = residual_add(f"dadd{idx}_2", cursor_x + 5.6, y)
            nodes += [ln2, xatt, add2]
            edges += [
            ...
            cursor_x = add2.x + 1.7
        ln3 = layernorm(f"dln{idx}_3", cursor_x, y)
        ff = ffn(f"dffn{idx}", cursor_x + 3.0, y + 0.2)
        add3 = residual_add(f"dadd{idx}_3", cursor_x + 5.4, y + 0.2)
        nodes += [ln3, ff, add3]
        edges += [
